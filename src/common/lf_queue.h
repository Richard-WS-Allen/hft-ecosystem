...
namespace Common {
    ...
    class LfQueue final {
      public:
        LfQueue(size_t queue_size) :
          store_(queue_size, T()), max_size_{queue_size} {}
        ...
        T* getNextWriteTo() noexcept {
          ...
          if (size() == max_size_) [[unlikely]] return nullptr;
        ...
        void updateNextRead() noexcept {
          next_read_i_ = (next_read_i_ + 1) %
            store_.size();
          ASSERT(elem_count_.load() != 0,
            "Invalid read in LfQueue.");
          elem_count_--;
        }
